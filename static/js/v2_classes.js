...
class Routing extends V2CommonClass{
    constructor(domainStrategy='IPIfNonMatch', rules=Routing.defaultRules()) {
        super();
        this.strategy = domainStrategy;
        this.rules = rules;
    }

    static defaultRules() {
        return [Routing.Rule.blockRule()];
    }

    static fromJson(json={}) {
        return new Routing(
            json.domainStrategy,
            json.rules.map(rule => Routing.Rule.fromJson(rule)),
        );
    }

    toJson() {
        return {
            domainStrategy: this.strategy,
            rules: Routing.toJsonArray(this.rules),
        };
    }
}

Routing.Rule = class extends V2CommonClass {
    constructor({
                    type='field', domain=[],
                    ip=[], port=[], network='',
                    source=[], inboundTag=[],
                    outboundTag=[], protocol=[]
                }) {
        super();
        this.type = type;
        this.domain = domain;
    ...
    static fromJson(json={}) {
        return new Routing.Rule({
            ...
            domain: json.domain,
            ip: json.ip,
            port: json.port.split(','),
            network: json.network,
            source: json.source,
            inboundTag: json.inboundTag,
    ...
    toJson() {
        return {
            type: this.type,
            domain: Rule.emptyOrDefault(this.domain),
            ip: Rule.emptyOrDefault(this.ip),
            port: Rule.emptyOrDefault(this.port, this.port.join(',')),
            network: Rule.emptyOrDefault(this.network),
            source: Rule.emptyOrDefault(this.source),
            inboundTag: Rule.emptyOrDefault(this.inboundTag),
            outboundTag: Rule.emptyOrDefault(this.outboundTag),
            protocol: Rule.emptyOrDefault(this.protocol),
        };
    }
};

class Outbound extends V2CommonClass {
    constructor(sendThrough='0.0.0.0',
    ...
    toJson() {
        return {
            sendThrough: this.through,
            protocol: this.protocol,
            settings: this.settings,
            tag: Outbound.emptyOrDefault(this.tag),
            streamSettings: isEmpty(this.stream) ? undefined : this.stream.toJson(),
            proxySettings: isEmpty(this.proxy) ? undefined : {tag: this.proxy},
        };
    }
}

Outbound.Settings = class extends V2CommonClass {
    constructor() {
        super();
    }
...
class V2Config extends V2CommonClass{
    constructor(inbounds=[new Inbound()],
                outbounds=[],
                routing=new Routing(),
                ) {
        super();
    ...
    static fromJson(json={}) {
        return new V2Config(
            json.inbounds.map(inbound => Inbound.fromJson(inbound)),
            json.outbounds.map(outbound => Outbound.fromJson(outbound)),
            Routing.fromJson(json.routing),
        );
    }

    toJson() {
        return {
            inbounds: V2Config.toJsonArray(this.inbounds),
            outbounds: V2Config.toJsonArray(this.outbounds),
